...
class EntriesCount:
    def find_entries(self, log_entries: dict, text: str, unwanted: str, date: str) -> dict:
        ...
        if unwanted:
            ...
            for unw in unwanted:
                log_entries = dict((key, value) for key, value in log_entries.items() if
                                   unw.strip().lower() not in value.lower())
        return log_entries
    ...
    def parse_entries(self, logs: list) -> dict:
        ...
        for log in logs:
            for line in log:
                possible_date = line[:23]  #TODO maybe better use endswith()!!!!
                try:
